...
package com.huoju.station.supernode.demo.context;
...
import com.huoju.station.supernode.demo.TestPrincipal;
...
public class ContextPortfolioControllerTests {
	...
	@Test
	public void subscribeClients() throws Exception{
		StompHeaderAccessor headers = StompHeaderAccessor.create(StompCommand.SUBSCRIBE);
		headers.setSubscriptionId("0");
		headers.setDestination("/topic/greetings");
		headers.setSessionId("0");
//		headers.setUser(new TestPrincipal("obs1"));
		headers.setSessionAttributes(new HashMap<String, Object>());
		Message<byte[]> message = MessageBuilder.createMessage(new byte[0], headers.getMessageHeaders());
		
		this.brokerChannelInterceptor.setIncludedDestinations("*");
		this.clientOutboundChannelInterceptor.setIncludedDestinations("*");
		
		this.clientInboundChannel.send(message);//in也是相对于容器的
		Thread.sleep(10000);
		StompHeaderAccessor headers2 = StompHeaderAccessor.create(StompCommand.SEND);
		headers2.setSubscriptionId("1");
		headers2.setDestination("/app/hello");
		headers2.setSessionId("1");
		headers2.setUser(new TestPrincipal("clientx"));
		headers2.setSessionAttributes(new HashMap<String, Object>());
		
		Message<byte[]> message2 = MessageBuilder.createMessage(new byte[0], headers2.getMessageHeaders());
		
		
		
		this.clientInboundChannel.send(message2);//in也是相对于容器的
		
		Message<?> reply = this.brokerChannelInterceptor.awaitMessage(5);
		Message<?> reply2 = this.clientOutboundChannelInterceptor.awaitMessage(5);
		assertNotNull(reply);
	...
	public void getClients() throws Exception {
		...
		StompHeaderAccessor headers = StompHeaderAccessor.create(StompCommand.SUBSCRIBE);
		headers.setSubscriptionId("0");
		headers.setDestination("/app/clients");
		headers.setSessionId("0");
		headers.setUser(new TestPrincipal("obs1"));
		headers.setSessionAttributes(new HashMap<String, Object>());
		Message<byte[]> message = MessageBuilder.createMessage(new byte[0], headers.getMessageHeaders());

		this.clientOutboundChannelInterceptor.setIncludedDestinations("/app/clients");//out是相对于容器的
		this.brokerChannelInterceptor.setIncludedDestinations("*");
		this.clientInboundChannel.send(message);//in也是相对于容器的
		...
		assertEquals("/app/clients", replyHeaders.getDestination());

		String json = new String((byte[]) reply.getPayload(), Charset.forName("UTF-8"));
		new JsonPathExpectationsHelper("$[0].name").assertValue(json, "111");
		new JsonPathExpectationsHelper("$[1].name").assertValue(json, "222");
		new JsonPathExpectationsHelper("$[2].name").assertValue(json, "333");
		
		
		StompHeaderAccessor headers2 = StompHeaderAccessor.create(StompCommand.SEND);
		headers2.setSubscriptionId("1");
		headers2.setDestination("/app/login");
		headers2.setSessionId("1");
		headers2.setUser(new TestPrincipal("clientx"));
		headers2.setSessionAttributes(new HashMap<String, Object>());
		...
		reply = this.brokerChannelInterceptor.awaitMessage(10);
		assertNotNull(reply);
		
		json = new String((byte[]) reply.getPayload(), Charset.forName("UTF-8"));
		new JsonPathExpectationsHelper("$[3].name").assertValue(json, "new one");
	}
...
//	}
	...
	@EnableScheduling
	@ComponentScan(
			basePackages="com.huoju.station.supernode.demo",
	...
	static class TestConfig implements ApplicationListener<ContextRefreshedEvent> {
		...
		public void onApplicationEvent(ContextRefreshedEvent event) {
			for (MessageHandler handler : handlers) {
				if (handler instanceof SimpAnnotationMethodMessageHandler) {System.out.println("*********waht");
					continue;
				}
				for (SubscribableChannel channel :channels) {System.out.println("*********are");
